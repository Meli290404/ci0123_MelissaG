...
/**
  ...
  *  hecho desde afuera (normalmente desde accept(), que trae un
  *  nuevo socket ya conectado al cliente). 
  *
  ...
  *      pasa la familia real del socket que estaba escuchando, para
  *      que un servidor IPv6 tambien funcione bien)
  *
 **/
void VSocket::Init( int id, bool ipv6 ){

   // si accept() fallo en algun punto anterior, no debería llegar acá
   if ( -1 == id ) {
      throw std::runtime_error( "VSocket::Init(int), descriptor invalido" );
   }

   this->sockId = id;
   this->IPv6 = ipv6;   // ya no se asume IPv4 a la fuerza, usamos lo que nos digan
...
/**
  ...
  *  Ahora nos fijamos en this->IPv6 para armar la estructura
...
/**
  ...
  *
  *  Este método no acepta ninguna conexion todavia, solo le dice al kernel
  *  "este socket va a ser un socket servidor, prepara una cola de tamaño
...
/**
  ...
  *
  *  Este metodo se queda "bloqueado" (el proceso se duerme) hasta que llegue un cliente nuevo.
  *  Cuando eso pasa, el kernel crea un descriptor nuevo (distinto al que esta escuchando) que es
  *  exclusivamente para hablar con ese cliente. El socket original (this->sockId) se queda libre para seguir escuchando mas conexiones.
  *
 **/
