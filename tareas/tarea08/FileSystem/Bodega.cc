...
bool Bodega::insertarStock(const std::string& idCategoria, const std::string& nombreProducto,
                            int cantidad, uint32_t precioCentavos) {
    std::lock_guard<std::mutex> lock(fs_.mutex());

    // primero se ubica la categoria, igual que en agregarProducto: sin la casilla
    // del directorio no se puede llegar al bloque de indice de esa categoria
    int bloqueCasilla, posCasilla;
    if (!ubicarCasillaCategoria(idCategoria, bloqueCasilla, posCasilla)) {
        std::cerr << "categoria no encontrada: " << idCategoria << "\n";
        return false;
    ...
    // recorre todos los bloques de datos que ya tiene la categoria buscando un
    // producto con ese nombre exacto; si lo encuentra, le suma la cantidad nueva
    // al stock que ya tenia en vez de crear un registro duplicado
    for (int b = 0; b < indice.cantidadBloques; b++) {
        RegistroProducto bloqueDatos[PRODUCTOS_POR_BLOQUE];
        fs_.leerBloque(indice.bloquesDatos[b], bloqueDatos);
        for (int p = 0; p < PRODUCTOS_POR_BLOQUE; p++) {
            if (bloqueDatos[p].estado == 1) {
                std::string nombreExistente(bloqueDatos[p].nombre, strnlen(bloqueDatos[p].nombre, sizeof(bloqueDatos[p].nombre)));
                if (nombreExistente == nombreProducto) {
                    bloqueDatos[p].cantidad += cantidad;
                    bloqueDatos[p].precioCentavos = precioCentavos; // se actualiza al precio mas reciente
    ...
    // si no existia el producto, se crea un registro nuevo: mismo bloque de
    // codigo que agregarProducto, buscando primero una ranura libre y, si no
    // hay, pidiendo un bloque nuevo al bitmap y agregandolo al indice
    int posicionLibre = -1;
    int bloqueDestino = -1;
    for (int b = 0; b < indice.cantidadBloques && bloqueDestino == -1; b++) {
        RegistroProducto bloqueDatos[PRODUCTOS_POR_BLOQUE];
...
bool Bodega::extraerStock(const std::string& idCategoria, const std::string& nombreProducto, int cantidad) {
    std::lock_guard<std::mutex> lock(fs_.mutex());
    ...
    // recorre los bloques de datos buscando el producto por nombre; a diferencia
    // de insertarStock, aca no se crea nada si no existe, solo se resta stock
    // de un producto que ya esta registrado
    for (int b = 0; b < indice.cantidadBloques; b++) {
        RegistroProducto bloqueDatos[PRODUCTOS_POR_BLOQUE];
        fs_.leerBloque(indice.bloquesDatos[b], bloqueDatos);
        for (int p = 0; p < PRODUCTOS_POR_BLOQUE; p++) {
            if (bloqueDatos[p].estado == 1) {
                std::string nombreExistente(bloqueDatos[p].nombre, strnlen(bloqueDatos[p].nombre, sizeof(bloqueDatos[p].nombre)));
                if (nombreExistente == nombreProducto) {
                    // se valida que haya suficiente stock antes de restar, para
                    // no dejar la cantidad en un numero negativo
                    if (bloqueDatos[p].cantidad < cantidad) {
                        std::cerr << "stock insuficiente: hay " << bloqueDatos[p].cantidad
    ...
    std::cerr << "producto no encontrado: " << nombreProducto << "\n";
    return false;
}