...
int VSocket::Bind( int port ) {
   ...
   if ( !IPv6 ) {
      ...
      // Misma idea que en IPv4, pero con sockaddr_in6 y sus campos sin6_*
      struct sockaddr_in6 host6;
      memset( (char *) &host6, 0, sizeof( host6 ) );
 
      host6.sin6_family = AF_INET6;
      host6.sin6_addr = in6addr_any;               // equivalente a INADDR_ANY pero en IPv6
      host6.sin6_port = htons( (uint16_t) port );
 
      st = bind( sockId, (struct sockaddr *) &host6, sizeof( host6 ) );
      if ( -1 == st ) {
         throw std::runtime_error( std::string( "VSocket::Bind, bind: " ) + strerror( errno ) );
      }
...
size_t VSocket::sendTo( const void * buffer, size_t size, void * addr ) {
   ...
   // El tamano de la estructura depende de si el socket es IPv4 o IPv6.
   socklen_t addrLen = IPv6 ? sizeof( struct sockaddr_in6 ) : sizeof( struct sockaddr_in );
...
size_t VSocket::recvFrom( void * buffer, size_t size, void * addr ) {
   ...
   // fromLen empieza con el tamano maximo que "addr" puede contener; recvfrom
   // lo actualiza con el tamano real de la direccion que efectivamente llego.
   socklen_t fromLen = IPv6 ? sizeof( struct sockaddr_in6 ) : sizeof( struct sockaddr_in );
 
   // recvfrom llena "addr" con la direccion de quien nos escribio
